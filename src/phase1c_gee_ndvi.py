"""
AGRI-2050: Faz 1c — Google Earth Engine NDVI Verisi
====================================================
Sentinel-2 ile Türkiye 81 il için 2004-2024 aylık NDVI.
Çalıştırma: python src/phase1c_gee_ndvi.py
====================================================
"""
...
def fetch_ndvi_il(il, lat, lon, start_year=2004, end_year=2024):
    """Sentinel-2 ile aylık ortalama NDVI"""
    point = ee.Geometry.Point([lon, lat]).buffer(25000)  # 25km buffer
    records = []
    ...
    for year in range(start_year, end_year + 1):
        for month in range(1, 13):
            ...
            try:
                ...
                records.append({
                    "il": il, "year": year, "month": month,
                    "ndvi": None, "source": "Sentinel-2 GEE"
                })
...
def run_ndvi_pipeline(raw_dir="data/raw", start_year=2004, end_year=2024):
    print("=" * 60)
    print("AGRI-2050 | GEE Sentinel-2 NDVI — 81 İl")
    print("=" * 60)
    print(f"  Dönem: {start_year}-{end_year} | {len(ILLER)} il\n")
    ...
    il_ndvi = full_df.groupby("il")["ndvi"].mean().sort_values()
    print(f"\n  En düşük NDVI : {', '.join(il_ndvi.head(3).index.tolist())}")
    print(f"  En yüksek NDVI: {', '.join(il_ndvi.tail(3).index.tolist())}")
    print("\n[✓] NDVI tamamlandı.\n")
    return full_df
